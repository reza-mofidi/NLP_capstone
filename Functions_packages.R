...
library(shiny); library(ggplot2); library(BH); library(NLP);library(tm); library(quanteda); library(dplyr); library(tidytext); library(ggplot2); library(ngram); library(wordcloud)
...
scoreNgrams <- function(x, nrows=30) {
  ...
  if (nrow(df) > 0) {
    ...
    # If result is less than 10, append highest frequency single word to it.
    if (nrow(df)<10) {
      rest <- 10-nrow(df)
